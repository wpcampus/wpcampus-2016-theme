...
class GWPreviewConfirmation {
	...
	public static function replace_merge_tags( $form ) {
		...
		foreach ( $form['fields'] as &$field ) {
			...
			if ( ! empty( $matches ) ) {
				...
				/*
				 * If default value needs to be replaced but
				 * is not on current page, wait until on the current page to replace it.
				 */
	...
	public static function preview_image_display( $field, $form, $value ) {
		...
		/*
		 * Need to get the tmp $file_info to retrieve real
		 * uploaded filename, otherwise will display ugly tmp name.
		 */
	...
	/**
	 * Retrieves $lead object from class if it
	 * has already been created; otherwise creates a new $lead object.
	 */
	...
	public static function preview_replace_variables( $content, $form ) {
		...
		/*
		 * Add filter that will handle getting temporary URLs
		 * for file uploads and post image fields (removed below).
		 *
		 * Beware, the RGFormsModel::create_lead() function also
		 * triggers the gform_merge_tag_filter at some point and will
